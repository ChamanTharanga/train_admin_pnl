			...
			<>
				 ...
				 */}
					...
					<div id="left-sidebar" className="sidebar">
						<h5 className="brand-name">Train Booking<span onClick={() => this.toggleAction()} className={`menu_option float-right${menuGrid ? ' active' : ''}`}><i
							className="icon-grid font-16" data-toggle="tooltip" data-placement="left"
							title="Grid & List Toggle"></i></span>
						</h5>
						<ul className="nav nav-tabs">
							<li className="nav-item"><span className={`nav-link ${!menuType ? ' active' : ''}`} data-toggle="tab" onClick={() => this.setState({ menuType: false })}>Booking</span></li>
